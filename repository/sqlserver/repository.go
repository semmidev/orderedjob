...
func (r *Repository) ReplayJobWithPayload(ctx context.Context, id uuid.UUID, newPayload json.RawMessage) error {
	...
	if len(newPayload) > 0 {
		res, err = r.db.ExecContext(ctx, `
			UPDATE ordered_jobs
			SET payload = ?, status = 'PENDING', attempt = 0, available_at = GETUTCDATE(), updated_at = GETUTCDATE()
			WHERE id = ? AND status IN ('FAILED', 'DEAD_LETTERED', 'CANCELLED')
		`, string(newPayload), id.String())
	} else {
		res, err = r.db.ExecContext(ctx, `
			UPDATE ordered_jobs
			SET status = 'PENDING', attempt = 0, available_at = GETUTCDATE(), updated_at = GETUTCDATE()
...
func (r *Repository) GetStats(ctx context.Context) (orderedjob.Stats, error) {
	...
	err = r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM ordered_jobs WHERE status IN ('PENDING', 'RETRYING') AND available_at > GETUTCDATE()`).Scan(&stats.Scheduled)
	if err != nil {
		return stats, err
	}
...
func (r *Repository) ListJobs(ctx context.Context, filter orderedjob.JobFilter) ([]orderedjob.Job, int64, error) {
	...
	if filter.ScheduledOnly {
		whereClauses = append(whereClauses, "status IN ('PENDING', 'RETRYING') AND available_at > GETUTCDATE()")
	}
	if filter.ChainID != "" {
...
func (r *Repository) buildDLQWhere(filter orderedjob.JobFilter) (string, []any) {
	...
	if filter.Search != "" {
		conds = append(conds, "(CAST(id AS NVARCHAR(36)) LIKE ? OR idempotency_key LIKE ? OR trace_id LIKE ? OR last_error LIKE ?)")
...
func (r *Repository) BulkReplayDLQ(ctx context.Context, filter orderedjob.JobFilter) (int64, error) {
	...
	query := fmt.Sprintf(`
		UPDATE ordered_jobs
		SET status='PENDING', attempt=0, last_error='', available_at=GETUTCDATE(), updated_at=GETUTCDATE(), lease_until=NULL
		WHERE %s
	`, whereStmt)
...
func (r *Repository) BulkSkipDLQ(ctx context.Context, filter orderedjob.JobFilter) (int64, error) {
	...
	updateQuery := fmt.Sprintf(`
		UPDATE ordered_jobs
		SET status='COMPLETED', completed_at=GETUTCDATE(), updated_at=GETUTCDATE(), lease_until=NULL
		WHERE %s
	`, whereStmt)
	...
	for _, it := range items {
		_, _ = tx.ExecContext(ctx, `
			UPDATE ordered_jobs
			SET status='PENDING', updated_at=GETUTCDATE()
			WHERE chain_id=? AND sequence=?+1 AND status='BLOCKED'
		`, it.chainID, it.seq)
	}
...
func (r *Repository) CountDLQ(ctx context.Context) (int64, error) {
	var count int64
	err := r.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM ordered_jobs WHERE status IN ('FAILED', 'DEAD_LETTERED')").Scan(&count)
	return count, err
}
